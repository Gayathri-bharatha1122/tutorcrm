  ...
  const pageContent: Record<string, React.ReactNode> = {
    'courses': (
      <div className="space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><BookOpen className="h-4 w-4 text-emerald-400" /> Enrolled Courses</h3>
          {[
            ...
            <div key={i} className="py-3 border-b border-slate-800 last:border-0">
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  <span className="text-[10px] text-slate-500">{c.tutor}</span>
                </div>
    ...
    'assignments': (
      <div className="space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><FileText className="h-4 w-4 text-indigo-400" /> My Assignments</h3>
          {[
            ...
            <div key={i} className="flex items-center justify-between py-3 border-b border-slate-800 last:border-0">
              ...
              <div className="text-right">
                {a.grade && <span className="text-xs font-bold text-emerald-400 block">{a.grade}</span>}
    ...
    'attendance': (
      <div className="space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><Calendar className="h-4 w-4 text-emerald-400" /> Monthly Attendance Log</h3>
          {[
            ...
            <div key={i} className="flex items-center justify-between py-3 border-b border-slate-800 last:border-0">
              <div>
                ...
                <span className="text-[10px] text-slate-500">{a.present}/{a.sessions} sessions attended</span>
              </div>
    ...
    'results': (
      <div className="space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><Award className="h-4 w-4 text-indigo-400" /> Exam Results</h3>
          {[
            ...
            <div key={i} className="flex items-center justify-between py-3 border-b border-slate-800 last:border-0">
              <div>
                <span className="text-xs font-bold text-white block">{r.exam}</span>
                <span className="text-[10px] text-slate-500">{r.date}</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-sm font-extrabold text-white font-mono">{r.score}%</span>
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">{r.grade}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 max-w-lg">
        <div className="flex items-center gap-5 mb-6">
          ...
          <div>
            ...
            <span className="text-xs text-emerald-400 font-bold uppercase">Student • 11th Grade</span>
          </div>
        </div>
        <div className="space-y-3">
          {[
            { label: 'Grade', value: '11th Grade' },
            { label: 'Enrolled Courses', value: '4 Subjects' },
            { label: 'Email', value: 'student@edumanage.com' },
            { label: 'GPA', value: '3.8 / 4.0' },
            { label: 'Class Rank', value: '#4 of 32' },
          ].map((f, i) => (
